...
public class ConsoleApp {
    ...
    // Create text box
    TextBox textBox = new TextBox().setMultiline(true);
    textBox.setPreferredSize(new TerminalSize(30, 10));
    contentPanel.addComponent(textBox);

    // Create text area
    TextBox outputTextArea = new TextBox().setReadOnly(true).setPreferredSize(new TerminalSize(30, 10));
    contentPanel.addComponent(outputTextArea);

    // Add key event listener to the text box
    textBox.setValidationPattern(Pattern.compile(".*")); // Allow any input
    textBox.addTo(com.googlecode.lanterna.input.KeyType.Enter, (event) -> {
        String enteredText = textBox.getText();
        outputTextArea.addLine(enteredText);
        textBox.setText("");
